...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Ritual Familiar popup window
    const ritualIcon = document.querySelector('.ritual-icon');
    const ritualPopup = document.getElementById('ritualPopup');
    const closeRitualBtn = document.getElementById('closeRitualPopup');
    if (ritualIcon && ritualPopup) {
        ritualIcon.addEventListener('click', () => {
            ritualPopup.style.display = ritualPopup.style.display === 'none' ? 'block' : 'none';
            if (ritualPopup.style.display === 'block') {
                ritualPopup.scrollIntoView({behavior: 'smooth'});
            }
        });
    }
    if (closeRitualBtn && ritualPopup) {
        closeRitualBtn.addEventListener('click', () => {
            ritualPopup.style.display = 'none';
        });
    }
    // Cerrar popup con la X
    const closeBtn = document.getElementById('closeRetoPopup');
    if (closeBtn) {
